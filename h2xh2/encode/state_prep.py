...
def get_non_ft_rz_plus_prep(phase: float, data_qubits: List[Qubit]) -> Circuit:
    ...
    c.H(data_qubits[0]).H(data_qubits[4]).H(data_qubits[6]).CX(
        ...
        phase, data_qubits[3], data_qubits[4]
    ).CX(
        data_qubits[3], data_qubits[2]
    )
    for q in data_qubits:
...
def get_prep_rz_part_ft_goto(
    ...
    c.append(
        iceberg_detect_zx(
            0, data_qubits, ancilla_qubits, syndrome_bits[1:3], discard_bits[0]
        )
    )
    c.append(
        iceberg_detect_zx(
            1, data_qubits, ancilla_qubits, syndrome_bits[3:5], discard_bits[1]
        )
    )
    c.add_clexpr(
        WiredClExpr(
    ...
    for _ in range(n_rus - 1):
        c.add_circbox(CircBox(goto_c), goto_c.qubits + goto_c.bits, condition=flag_bit)
    return c
